...
export default function BlogsPage() {
  ...
  const { mergeOne, translateIssue } = useBlogListAutoAr(posts, lang);
    ...
    <div key={lang} className="min-h-screen text-neutral-900">
      ...
      <main className="relative z-[1] pt-[72px] lg:pt-[80px]">
        <div className="max-w-[1200px] mx-auto section-padding py-10 sm:py-12 lg:py-14">
          <header className="mb-10 sm:mb-12 text-start">
            ...
            <p className="mt-3 text-neutral-400 text-sm sm:text-base max-w-md">{bt.description}</p>
          </header>

          {lang === "ar" && translateIssue === "no_api_key" && (
            <p
              className="mb-8 text-sm text-amber-900 bg-amber-50 border border-amber-100 rounded-xl px-4 py-3 max-w-2xl"
              dir="ltr"
            >
              {bt.translateNotConfigured}
            </p>
          )}
          {lang === "ar" && translateIssue === "translate_error" && (
            <p className="mb-8 text-sm text-amber-900 bg-amber-50 border border-amber-100 rounded-xl px-4 py-3 max-w-2xl">
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-10 items-center">
                    ...
                    <div className="lg:col-span-5 flex flex-col justify-center">
                      ...
                      <h2 className="text-[1.35rem] sm:text-[1.5rem] font-bold text-neutral-900 leading-tight tracking-[-0.02em] mb-3 group-hover:text-vibo-primary transition-colors">
                        {pickBlogTitle(mergeOne(featured), lang)}
                      </h2>
                      <p className="text-[0.95rem] text-neutral-600 leading-relaxed line-clamp-4">
                        {pickBlogExcerpt(mergeOne(featured), lang)}
                      </p>
                ...
                {rest.map((post) => (
                  <article key={post._id} className="flex flex-col h-full">
                    <Link href={`/blogs/${post.slug}?lang=${lang}`} className="group flex flex-col h-full">
                      ...
                      <p className="text-[0.85rem] text-neutral-600 line-clamp-2 mb-4 flex-1">
