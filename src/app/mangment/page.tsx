...
import NewsroomModeration from "@/components/newsroom/NewsroomModeration";
import { getTranslations, isRTL } from "@/i18n";
import { useViboLang } from "@/i18n/useViboLang";

export default function MangmentPage() {
  const { lang, switchLang } = useViboLang();
  const t = getTranslations(lang);
  const rtl = isRTL(lang);
  const blogUi = t.blogManagement;
  ...
  return (
    <div className="min-h-screen bg-[#fafafa] text-neutral-900">
      ...
      <main className="relative z-[1] pt-[72px] lg:pt-[80px] pb-16">
        <div className="max-w-[1100px] mx-auto section-padding py-10 sm:py-12 space-y-10">
          <BlogManagement ui={blogUi} />
          <NewsroomModeration lang={lang} />
        </div>
      </main>
