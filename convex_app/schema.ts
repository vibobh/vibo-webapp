...
/** Blog CMS categories (see `blogs.ts`). */
...
export default defineSchema({
  users: defineTable({
    ...
    /** Web auth / admin (Next.js JWT + staff tooling). */
  ...
  stories: defineTable({
    ...
    /** @mention sticker — tags a single user (`stories.ts`). */
    ...
    /** “Notify Me” sticker — viewers subscribe for post alerts. */
    ...
    /** Music sticker — Spotify track preview clip. */
    ...
    /** Reusable layout template (`storyTemplates.ts`) when the author saved this story as a template. */
    templateId: v.optional(v.id("storyTemplates")),
    createdAt: v.number(),
    expiresAt: v.number(),
  })
  ...
  storyQuizAnswers: defineTable({
    ...
    .index("by_story_user", ["storyId", "userId"])
    .index("by_story", ["storyId"]),
  /** Notify-me sticker: one row per subscriber per story (`stories.ts`). */
  storyNotifySubscriptions: defineTable({
    storyId: v.id("stories"),
    creatorId: v.id("users"),
    subscriberId: v.id("users"),
    createdAt: v.number(),
  }).index("by_story_subscriber", ["storyId", "subscriberId"]),
  /** Emoji-slider sticker: one vote per viewer per story. */
  storyEmojiSliderVotes: defineTable({
    storyId: v.id("stories"),
    creatorId: v.id("users"),
    voterId: v.id("users"),
    value: v.number(),
    ...
    .index("by_story_voter", ["storyId", "voterId"])
    .index("by_story", ["storyId"]),
  /** Saved story layout templates (`storyTemplates.ts`). */
  storyTemplates: defineTable({
    ...
    .index("by_creator", ["creatorUserId", "createdAt"])
    .index("by_source_story", ["sourceStoryId"])
    .index("by_status_uses", ["status", "usesCount"]),
  ...
  notificationGroups: defineTable({
    ...
    type: v.union(
      ...
      /** Multi-collaborator roster invite (see `postCollaborators.ts`). */
  ...
  conversations: defineTable({
    ...
    createdAt: v.number(),
    updatedAt: v.number(),
    /** DM chat pin (visible to all participants, Instagram-style). */
  ...
  messages: defineTable({
    ...
    /** Roster collab invite row when `type === "collab_invite"` from publish flow. */
    collaborationInviteId: v.optional(v.id("postCollaborators")),
    ...
    /** Denormalized author of the quoted message (for “You replied to …” UI). */
    replyToSenderId: v.optional(v.id("users")),
    /** Quick emoji reactions on a message (viewer toggles one emoji per user). */
  ...
  posts: defineTable({
    ...
    mediaCount: v.number(),
    /** Optional display line for attached music (catalog TBD). */
    musicTitle: v.optional(v.string()),
    /** Co-author: invitee must accept in DMs before the post can publish. */
    collaborationInviteeId: v.optional(v.id("users")),
    ...
    /** Creator + accepted collaborators for feed follow-graph matching (`postDistribution.ts`). */
  ...
  /** Multi-collaborator invites on a post (publish-time roster; DMs). See `postCollaborators.ts`. */
  postCollaborators: defineTable({
    postId: v.id("posts"),
    creatorUserId: v.id("users"),
    ...
    /** Where the row was created (extend union when new flows are added). */
    source: v.union(v.literal("post_creation")),
    invitedAt: v.number(),
    respondedAt: v.optional(v.number()),
    dmClientMessageId: v.optional(v.string()),
    /** Qualified views attributed to this collaborator (`productAnalytics.ts`). */
    attributedQualifiedViews: v.optional(v.number()),
    createdAt: v.number(),
    updatedAt: v.number(),
  })
    .index("by_post", ["postId"])
    .index("by_post_collaborator", ["postId", "collaboratorUserId"]),

  /**
   * Append-only counter deltas rolled into `posts` by `rollupPostCounterDeltas`
   * (`postCounterDeltas.ts`).
   */
  postCounterDeltas: defineTable({
    postId: v.id("posts"),
    field: v.union(
      v.literal("likeCount"),
  ...
  }).index("by_processed_created", ["processedAt", "createdAt"]),

  /** Spotify client-credentials token cache (singleton row; see `music.ts`). */
  spotifyTokenCache: defineTable({
  ...
  // ============================================
  // DRAFT UPLOADS — Instagram-style optimistic background uploads
  // ============================================

  /**
   * Background uploads kicked off as soon as the user enters the composer.
   * Bytes go straight to S3 under `posts-pending/{userId}/{uploadId}/...`,
   * and on Share the row is flipped to `published` and the same key is reused
   * by `postMedia` (no S3 CopyObject). Cleanup is purely cron-driven on rows
   * still in a non-published terminal state past `expiresAt`.
   */
  draftUploads: defineTable({
    /** Stable client-generated id; survives composer remounts. */
    uploadId: v.string(),
    userId: v.id("users"),
    /** Groups all uploads belonging to one composer session. */
    composerSessionId: v.string(),
    mediaType: v.union(v.literal("image"), v.literal("video")),
    /** Mime of the main asset. */
    fileType: v.string(),
    /** Permanent S3 key (under posts-pending/) — reused on publish, no copy. */
    s3Key: v.string(),
    s3Region: v.optional(v.string()),
    /** Optional sibling thumbnail (videos). */
    thumbnailKey: v.optional(v.string()),
    thumbnailRegion: v.optional(v.string()),
    /** Resolved dimensions / metadata after client transcode. */
    width: v.optional(v.number()),
    height: v.optional(v.number()),
    durationMs: v.optional(v.number()),
    hasAudioTrack: v.optional(v.boolean()),
    ...
    error: v.optional(v.string()),
    createdAt: v.number(),
    /** Cleanup cron sweeps non-`published` rows past this. */
    expiresAt: v.number(),
  })
    .index("by_user_session", ["userId", "composerSessionId"])
  ...
  contentHashes: defineTable({
    ...
    postId: v.id("posts"),
    hash: v.string(),
    /** "media" / "caption" / "bundle" (full-post fingerprint; not mirrored to globalContentHashes). */
    hashType: v.union(
      v.literal("media"),
      v.literal("caption"),
      v.literal("bundle"),
  ...
  /** Context-card match candidates between content-intelligence jobs (see `contextCardCandidatesDb.ts`). */
  contextCardCandidates: defineTable({
    ...
    .index("by_sourceContentId", ["sourceContentId"])
    .index("by_sourcePostId", ["sourcePostId"])
    .index("by_targetPostId", ["targetPostId"])
    .index("by_status", ["status"]),

  /**
   * Async content-intelligence jobs (transcripts, embeddings, summaries).
   * See `contentIntelligenceDb.ts` / `contentIntelligence.ts`.
   */
  contentIntelligence: defineTable({
    contentId: v.string(),
    contentType: v.union(
      v.literal("video"),
      v.literal("image"),
    ...
    confidenceOverall: v.optional(v.number()),
    createdAt: v.number(),
    updatedAt: v.number(),
  })
  ...
  /** Marketing / CMS blog posts (see `blogs.ts`). */
  blogs: defineTable({
    slug: v.string(),
    title: v.string(),
    excerpt: v.string(),
